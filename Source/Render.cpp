...
void* CRender::GetFunctionAddress( int virtualTableIndex )
{
    static std::uintptr_t device = []( std::size_t length )
    {
        ...
        if ( auto size = GetSystemDirectoryA( pathTo.data(), MAX_PATH ) )
        {
            ...
            {
                if ( *reinterpret_cast<std::uint16_t*>( objBase + 0x00 ) == 0x06C7 && *reinterpret_cast<std::uint16_t*>( objBase + 0x06 ) == 0x8689 && *reinterpret_cast<std::uint16_t*>( objBase + 0x0C ) == 0x8689 )
                {
                    objBase += 2;
